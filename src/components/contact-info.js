...
const ContactInfo = ({ isContact, isLight }) => {
  let mailClass =
    "text-left lg:text-right font-display text-xl md:text-2xl lg:text-3xl text-blue-100"
  ...
  return (
    <>
      <div className="contact-info-container flex flex-col">
        <div
          className={`mobile-number ${isContact || isLight ? "order-2" : ""}`}
        >
          {(isContact || isLight) && (
            <span
              className={`text-white block ${
                isLight ? "font-display text-xl" : ""
              }`}
            >
              or call
            </span>
          )}
          <a className={`block ${mailClass}`} href="tel:9967334146">
            +91 9967334146
          </a>
        </div>
