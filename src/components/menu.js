      ...
      render ={data => data.allStrapiPage.nodes.filter(e => !e.strapiParent).sort((a,b) => a.order - b.order).map((page) => {
        return (
          <li key={page.strapiId} className="mb-8">
            <Link className="text-4xl md:text-6xl text-left menu-link md:text-right overflow-hidden" activeClassName="active" to={`/${page.slug}`}>
              <span className={`menu-link-wrap block`}>
             <span className="solid text-white block uppercase">{page.name}</span> 
             <span className="border-text uppercase block">{page.name}</span> 
              </span>
              </Link>
          </li>
        )
