...
const Navigation = ({ active, data, changeColor, windowEl }) => {
      ...
      lists.push(
        <li
          ...
          className={`mx-12 text-white text-lg tracking-body opacity-50 hover:opacity-100 transition-opacity  duration-500 ease-in-out ${
            list.id === active ? "active opacity-100" : ""
          }`}
        >
          <Link
            to={`/${list.slug}`}
  ...
  return (
    <nav
      className={`p-3 top-0 transition ease-in-out duration-500 sticky z-20 ${
        windowEl.scrollY > changeColor ? "bg-black" : "bg-blue-100"
      }`}
    >
